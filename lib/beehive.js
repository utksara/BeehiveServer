...
const { map_to_svg_values, register_3 } = require('./visualization.js');
...
let execution_order = beehiveUtils.execution_order;
...
const DebugParamValues = (system)=>{
    for (key in system){
        if (!(_optional_params.includes(key) || _core_params.includes(key))){
            DebugParams(system.ID, " : ", key, " :: ", system[key]);
        }
    } 
}
...
const _all_preceding_syss_ran  = function(system){
    return _.reduce(system.preceding_syss, function(result, value) {
        result = result && value; 
        return result;
    }, true);
}
...
async function run_system(system_id, fullfillment_criteria, 
    ...
    if (config.ADDTRACER){
        execution_order.push({NAME : sys_by_id[system_id].NAME, ID: system_id})
    }

    let system = sys_by_id[system_id]


    DebugParamValues(system);
    /**
    ...
    try {
        ...
        if (system.TO.length > 0){
            await _.reduce(system.TO, async function(result, subsys_id){
                let subsys = sys_by_id[subsys_id];
                subsys.preceding_syss[system.ID] = true;
                if(fullfillment_criteria(subsys)){
                    // DebugAny("help ")    
                    await run_system(subsys_id, fullfillment_criteria, results );
                }
                return result;
            }, []);
        }else{
    ...
    register_3 (system, vis_objs)
    // return await register(vis_objs, system);
}
...
let SYSTEM = function(initializer = {}){
    ...
    initializer.accumulables = initializer.accumulables ? initializer.accumulables : []
...
const RUNSIMULATION = async function (main, starting_system, fullfillment_criteria = _all_preceding_syss_ran){
    try{
        shapes._reset()
        main();
        await run_system(starting_system.ID, fullfillment_criteria);
        ...
        let calculate_all = async (interm_vis_objs) =>{
            ...
            for (let i = 0; i < data_length; i++){
                ...
                processed_vis_objs.stroke[i] = colorMap(Number(interm_vis_objs.point.quantity[i]), max_quant, min_quant)
            }
            DebugVisObject(processed_vis_objs.stroke)
...
const CONNECTIONS = async function (main, starting_system){
    try{
        ...
        // console.log("pusheen data", vis_objs);
