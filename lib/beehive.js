...
async function run_system(system, fullfillment_criteria){
    ...
    if (system.TO.length > 0){
        await _.reduce(system.TO, async function(result, subsys_id){
            ...
            if(fullfillment_criteria(subsys)){
                await run_system(subsys, fullfillment_criteria);
            }
            return result;
        }, []);
    }else{
...
const RUNSIMULATION = async function (main, starting_system, fullfillment_criteria = _all_requirements_fullfilled){
    try{
        shapes._reset()
        main();
        let data = await run_system(starting_system, fullfillment_criteria);
