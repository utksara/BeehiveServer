...
const { reset_run_status } = require('./core/systemUtils.js');
...
/**
 * This condition ensures all the parent systems have run_status true before running the current system
 * @param {current system} system 
 * @returns 
 */
const _all_preceding_syss_ran  = function(system){
    if (system.run_status == false){
        /**
         * Since the run_status is of type Set, we first need to convert it to Array before applying _.reduce 
         */
...
const RUN = async (system_id, fullfillment_criteria = _all_preceding_syss_ran) => {
    reset_run_status();
    run_system(system_id, fullfillment_criteria);
} 
