...
async function _run_system(system){
    ...
    if (system.TO.length > 0){
        const sub_process = _.reduce(system.TO, function(result, subsys_id){
            let subsys = sys_by_id[subsys_id];
            subsys.preceding_syss[system.ID] = true;
            if(_all_requirements_fullfilled(subsys)){
                // await _run_system(subsys);
                result.push(_run_system(subsys));
            }
            return result;
        }, []);
        await Promise.resolve(sub_process);
    }else{
        DebugProcesses("no sub process for ", system.ID);
    }

    console.log("pusheen calling register")
...
const CONNECTIONS = async function (main, starting_system){
    shapes._reset()
    main();
    console.log("pusheen  calling run systems");
    let data = await _run_system(starting_system);
    console.log("pusheen  array of items", data);
    // fs.writeFileSync('inputoutput/output_data.json', data, (err) => {
    //     if (err) throw err;
