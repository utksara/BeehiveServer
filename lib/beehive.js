...
let _push_sys = (direction) =>{
    return (sys_to_be_pushed)=>{
        if (direction === 'forward'){
            return (P)=>{
                P = _with_endpoints_sys(P)
                P.endpoints.forEach(sys => {
                    CONNECT (sys)(sys_to_be_pushed);
                });
                P = _with_endpoints_sys(P)
                return P;
            }
...
function _dfs_search(sys, by, values = [], accum = {}){
    ...
    sys.TO.forEach( subsys => {
        _dfs_search(subsys, by, values, accum);
    });
}

let _with_endpoints_sys = function(P){
    let sample_sys, tempsys;
    if (typeof P.endpoints === 'undefined' || P.endpoints.length === 0){
        sample_sys = [P];
    }else{
        sample_sys = P.endpoints
    }
    tempsys = sample_sys
    while(tempsys[0].TO.length > 0){
        sample_sys = tempsys[0].TO;
        tempsys = tempsys[0].TO
    }
    P.endpoints = sample_sys;
    return P;
}
...
async function _run_system(sys){
     ...
     */
    if (sys.TO.length > 0){
        const sub_process = _.reduce(sys.TO, function(result, subsys){
...
function copySystem(S, corresponding_system = {}){
    let Z = SYSTEM()
    for (element in S) {
            ...
            if (element === "TO"){
                S.TO.forEach(child => {
                    if (!Object.keys(corresponding_system).includes(child.ID) ){
                        const new_child = copySystem(child, corresponding_system) 
...
function _relocate_sys(sys_to_be_relocated, new_sys, _relocate_tracker = {}){
    ...
    for(let i = 0; i < sys_to_be_relocated.TO.length; i++) {
        let subsys = sys_to_be_relocated.TO[i] 

        // sys_to_be_relocated.TO.forEach(element => {
        //     DebugAny(sys_to_be_relocated.ID ,"-->", element.ID)   
        // });
        // DebugAny("----")

        if (Object.keys(_relocate_tracker).includes(subsys.ID)){
            let new_subsys = sys_by_id[_relocate_tracker[subsys.ID]]
            CONNECT(new_sys) (new_subsys);
...
let SYSTEM = function(param = {}){
    param.ID = _new_id()
    param.PROCESS = param.PROCESS ? param.PROCESS : []
    ...
    if (param.VISUALIZE.length > 0){
        param.VISUALIZE.forEach(element => {
            element._param = {}
            element._param.center = element.POSITION ? element.POSITION : shapes._init_pos();
            element._param.movement = element.REPRESENT ? param[element.REPRESENT] : "100, 100, 100, 100"
        });
    }
...
const BICONNECT = function() {
    ...
    return function (){
        ...
        console.log(1)
        CONNECT (sys_array_one) (sys_array_two)
        CONNECT (sys_array_two) (sys_array_one)
    }
}

const CONNECTADVANCED = function(){
    const From  = arguments
    return function (){
...
const SHELF = function (sys, N, params){
    let new_sys = bypassSystem(sys, {NAME : "Shelved" + sys.NAME})
    let copy_sys = copySystem(sys);
    for (let i = 0; i<N; i++){
        for (key in params){
            copy_sys[key] = params[key][i];
        }
        CONNECT (new_sys) (copy_sys);
        copy_sys = copySystem(copy_sys);
        let sys_iterator = copy_sys;
        _transform_postion_recursive(sys_iterator, [0,8]);
        // while(sys_iterator.TO.length >0){
        //     sys_iterator.TO.forEach(elementouter => {
        //         _transform_postion(elementouter, [0,1]);
        //     });
        //     sys_iterator = sys_iterator.TO[0];
        // }
    }
    DebugTOMap(new_sys);
    return new_sys;
...
const DISCONNECT = function() {
    ...
    return function (){
        for (let i in From) {
            for (let j in arguments){
                From[i].TO.splice(arguments[j], 1);
...
const CHAIN = function (sys, N){
    ...
    for (let i = 0; i<N; i++){
        ...
        _push_sys('forward')(copy_sys)(new_sys);
    }
    DebugTOMap(new_sys);
    return new_sys
}
...
const PATTERN = function (sys, fusion_map, N){
    ...
    let old_sys = copySystem(sys);
    let saved_sys = old_sys
    let displacement = [1,0]
    for (let i = 0; i<N-1; i++){   
        let new_sys = copySystem(old_sys)
...
function MESH(sys, m , n){
    let new_sys = SYSTEM({NAME : "Shelved" + sys.NAME})
    let copy_sys = CHAIN(sys, n);
    ...
    for (let i = 0; i<m-1; i++){
        ...
        for(let j = 0; sys_iterator_final.TO.length >0; j++){
            sys_iterator_final.TO.forEach(elementouter => {
                _transform_postion(elementouter, [0,1]);
            });

            CONNECT (sys_iterator_initial) (sys_iterator_final);
            sys_iterator_final = sys_iterator_final.TO[0];
            sys_iterator_initial = sys_iterator_initial.TO[0];
        }
        CONNECT (sys_iterator_initial) (sys_iterator_final);
    }
