...
let global_id = beehiveUtils.global_id;
...
const _all_requirements_fullfilled  = function(system){
...
const _transform_postion = function (system, coordinates){
    if (_is_sys(system)){
...
const _transform_postion_recursive = function (system, coordinates){
    let eff = function (S) {
...
async function run_system(system, fullfillment_criteria = _all_requirements_fullfilled){
    ...
    const new_vis_objs = await register(vis_objs, system);
    return new_vis_objs;
}
...
let SYSTEM = function(param = {}){
    ...
    const message = 'system created' + JSON.stringify(param);
    logger.info(message);
    return param;
}
...
const CONNECTIONS = async function (main, starting_system){
    try{
        shapes._reset()
        main();
        let data = await run_system(starting_system);
        console.log("pusheen data", starting_system);
        fs.writeFileSync('inputoutput/output_data.json', JSON.stringify(data), (err) => {
            if (err) throw err;
...
const CHAIN = function (system, N, flow_parameters = []){
    ...
    for (let i = 0; i<N-1; i++){
        ...
        _transform_postion_recursive(system, [1,0]);
